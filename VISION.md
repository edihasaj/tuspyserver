...
- Triage and label every open issue and PR; ask for detail when a report is sparse.
- **Dependency-bump PRs** (dependabot) are Merge-by-Default candidates: label them
  `pr: 👀 needs review` for a quick human merge once CI is green — do not auto-merge.
- Auto-build a fix PR only for clear, in-scope bug fixes (Merge by Default); open it
  with a `Fixes #N` keyword.
- Escalate (`needs: 🙋 maintainer`) anything in Needs Sign-Off — protocol/API
  changes, new backends, breaking changes — instead of implementing it.
